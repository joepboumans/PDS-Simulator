...
public:
  ...
  uint32_t iter = 0;
  EM_FCMS(vector<vector<vector<vector<vector<uint32_t>>>>> &thresh,
    ...
    std::cout << "[EM_FCM] Init EMFSD" << std::endl;
    ...
    std::cout << "[EM_FCM] Finished setting up counter_dist and "
                 "thresholds"
              << std::endl;
    ...
    std::cout << "[EM_FCM] Initial cardinality guess : "
              << this->n_new << std::endl;
    ...
    std::cout << "[EM_FCM] Initial Flow Size Distribution guess"
              << std::endl;
    ...
    std::cout << "[EM_FCM] Summed Flow Size Distribution"
              << std::endl;
    /*for (auto &x : this->dist_new) {*/
    /*  if (x != 0) {*/
    /*    std::cout << x << " ";*/
...
private:
  ...
  void calculate_degree(vector<double> &nt, int d, int xi) {
    ...
    printf("[EM_FCM] ******** Running for degree %2d with a size of "
           "%12zu\t\t"
           "**********\n",
    ...
    if (this->counters[d][xi].size() != 0)
      printf("[EM_FCM] ******** depth %d degree %2d is "
...
public:
  void next_epoch() {
    std::cout << "[EM_FCM] Start next epoch" << std::endl;
    auto start = std::chrono::high_resolution_clock::now();

    this->n_old = this->n_new;
    this->dist_old = this->dist_new;

    std::cout << "[EM_FCM] Copy first degree distribution"
              << std::endl;
    ...
    std::cout << "[EM_FCM] Init first degree" << std::endl;
    // Simple Multi thread
    ...
    std::cout << "[EM_FCM] Created " << total_degree << " threads"
              << std::endl;

    for (size_t d = 0; d < DEPTH; d++) {
      for (size_t t = 2; t < threads[d].size(); t++) {
        std::cout << "[EM_FCM] Start thread " << t << " at depth "
                  << d << std::endl;
        threads[d][t] = std::thread(&EM_FCMS::calculate_degree, *this,
                                    std::ref(nt[d][t]), d, t);
      }
    }

    std::cout
        << "[EM_FCM] Started all threads, wait for them to finish..."
        << std::endl;
    ...
    std::cout << "[EM_FCM] Finished calculating nt per degree"
              << std::endl;
    ...
    printf("[EM_FCM - iter %2d] Compute time : %li\n", iter,
           time.count());
    printf("[EM_FCM - iter %2d] Intermediate cardianlity : "
           "%9.1f\n\n",
           iter, this->n_new);
    iter++;
  }
