...
template <int D, int W0, uint32_t THRESL1, uint32_t THRESL2> class EM_FCM {
...
public:
  ...
  set_counters(vector<vector<vector<uint32_t>>> _newsk,
    ...
    n_new = n_new / static_cast<double>(D); // normalize
    ...
    for (int j = 0; j < dist_new.size(); ++j) {
      dist_new[j] /= (static_cast<double>(D) * static_cast<double>(n_new));
      ns[j] /= static_cast<double>(D);
    }
  ...
  void next_epoch() {
    ...
    dist_old = dist_new;
    n_old = n_new;
    vector<double> denominator(D, 0.0);
    std::fill(ns.begin(), ns.end(), 0.0);
    vector<int> num_thread(D, 0); // for multi-threading
    ...
    vector<vector<vector<double>>> nt_d_xi(
        D); // d(depth)->xi(degree)->counts(num.flows)
    for (int d = 0; d < D; ++d)
    ...
    for (int i = 0; i < ns.size(); ++i) {
      if (ns[i] != 0) {
        ns[i] /= double(D); // divide the denominator
