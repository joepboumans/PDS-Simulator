...
public:
  ...
  set_counters(vector<vector<vector<uint32_t>>> _newsk,
    ...
    for (int d = 0; d < FCM_DEPTH; ++d) {                     // depth
    ...
    for (int j = 0; j < dist_new.size(); ++j) {
      dist_new[j] /=
          (static_cast<double>(FCM_DEPTH) * static_cast<double>(n_new));
  ...
  void next_epoch() {
    ...
    dist_old = dist_new;
    n_old = n_new;
    vector<double> denominator(FCM_DEPTH, 0.0);
    std::fill(ns.begin(), ns.end(), 0.0);
    vector<int> num_thread(FCM_DEPTH, 0); // for multi-threading
    ...
    vector<vector<vector<double>>> nt_d_xi(
        FCM_DEPTH); // d(depth)->xi(degree)->counts(num.flows)
    for (int d = 0; d < FCM_DEPTH; ++d)
    ...
    if (1) {
      for (int d = 0; d < FCM_DEPTH; ++d) {
        for (int xi = 1; xi <= num_thread[d]; ++xi) { // degree (xi)
          ...
          if (xi == 1) {
            thrd[iter_thread] =
                std::thread(&EM_WFCM::epoch_parallel_singledeg, *this,
                            std::ref(nt_d_xi[d][xi]), d, xi);
          } else {
            thrd[iter_thread] =
                std::thread(&EM_WFCM::epoch_parallel_multideg, *this,
                            std::ref(nt_d_xi[d][xi]), d, xi);
          }
          iter_thread += 1;
        }
