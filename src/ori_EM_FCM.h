...
template <int DEPTH, int W0, uint32_t THRESL1, uint32_t THRESL2> class EM_FCM {
...
private:
  ...
  double get_p_from_beta(BetaGenerator &bt, double lambda,
                         vector<double> &now_dist, double now_n, int d) {
  ...
  struct BetaGenerator_highdeg // for degree > 1, specifically for FCM-Sketch
  {
    ...
    explicit BetaGenerator_highdeg(int _sum, vector<vector<uint32_t>> _thres,
      ...
      // version that prohibits the number of flows less than D. As explained in
      // the paper, its effect is quite limited as the number of such counters
      // are only a few.
          ...
          if (get_new_comb()) {
            ...
            if (simplified == 0) {
              if (condition_check_fcm())
                return true;
            } else if (simplified == 3 or beta_degree == 2 or
                       beta_degree == 3) {
    ...
    bool condition_check_fcm() {
      ...
      if (beta_degree == now_flow_num) {
        ...
        // degree 2, num 3
        // layer 1
        if (now_result[0] + now_result[1] <= THRESL1 or
            now_result[2] <= THRESL1)
          return false;
        ...
        if (now_result[3] > THRESL1) {
          ...
          if (now_result[0] + now_result[3] <= THRESL1 or
              now_result[1] + now_result[2] <= THRESL1)
            return false;
        }
    ...
    bool condition_check_fcm_simple4to3() {
      ...
      if (num_cond_l2 == 0) { // if no layer 2 condition
        ...
        if (now_result[0] + now_result[1] <= THRESL2 + 2 * THRESL1 or
            now_result[2] <= THRESL2 + THRESL1)
          return false;
    ...
    bool condition_check_fcm_simple2() {
      ...
      // layer 2
      int num_cond_l2 =
          thres.size() - simplified; // here, simplified is origianl degree
      if (num_cond_l2 == 0) {        // if no layer 2 condition
        return true;
      } else if (num_cond_l2 == 1) { // if one condition
        int val = 0;
        ...
        if (simplified >= 4 or beta_degree == 3) {
          if (val <= THRESL2 + 3 * THRESL1) {
            return false;
          }
        } else {
          if (val <= THRESL2 + 2 * THRESL1) {
            return false;
          }
        }
      } else if (num_cond_l2 == 2 and beta_degree == 3) { // if two conditions
        if (now_result[0] + now_result[1] <= THRESL2 + 2 * THRESL1 or
            now_result[2] <= THRESL2 + THRESL1)
          return false;
        ...
        if (now_result[0] <= THRESL2 + THRESL1 or
            now_result[1] <= THRESL2 + 2 * THRESL1) {
          return false;
        }
      }
      return true;
  ...
  double get_p_from_beta_fcm(BetaGenerator_highdeg &bt, double lambda,
                             vector<double> &now_dist, double now_n, int degree,
                             int d) {
    unordered_map<uint32_t, uint32_t> mp;
    for (int i = 0; i < bt.now_flow_num; ++i) {
      mp[bt.now_result[i]]++;
  ...
  void collect_counters(vector<vector<vector<uint32_t>>> counters) {
    ...
    for (int d = 0; d < DEPTH; ++d) {
      counter_dist[d].resize(
          counters[d].size()); // resize for possible degree size
      for (int xi = 0; xi < counters[d].size(); ++xi) {
        // xi is degree, but here we only use xi == 1
        if (counters[d][xi].size() == 0)
          continue; // stop and go next
        int max_counter_val = (int)(*std::max_element(counters[d][xi].begin(),
                                                      counters[d][xi].end()));
        this->max_val =
            (this->max_val > max_counter_val ? this->max_val : max_counter_val);
        counter_dist[d][xi].resize(
            max_counter_val +
            1); // resize for possible value size for each degree
        std::fill(counter_dist[d][xi].begin(), counter_dist[d][xi].end(),
                  0); // initialize to 0
        for (int j = 0; j < counters[d][xi].size(); ++j)
          counter_dist[d][xi]
                      [counters[d][xi][j]]++; // initialize counter dist (n_i)
        m[d][xi] = counters[d][xi].size(); // number of counters of each degree
                                           // except empty counters
      }
    }
  }
...
public:
  ...
  void
  set_counters(vector<vector<vector<uint32_t>>> _newsk,
               vector<vector<vector<vector<vector<uint32_t>>>>> _newsk_thres) {
    this->newsk = _newsk;
  ...
  void epoch_parallel_singledeg(vector<double> &nt_vec, int d, int xi) {
    ...
    for (int i = 1; i < counter_dist[d][xi].size(); ++i) { // start size 1
      ...
      while (bts1.get_next()) {
        double p = get_p_from_beta(bts1, lambda, dist_old, n_old, d);
        sum_p += p;
        imm_p.push_back(p);
        vector<double> p_results(std::begin(bts1.now_result),
                                 std::end(bts1.now_result));
      ...
      if (sum_p == 0) {
        ...
        for (size_t j = 0; j < imm_p.size(); j++) {
          ...
          for (size_t x = 0; x < imm_now_results[j].size(); ++x) {
            nt_vec[imm_now_results[j][x]] += counter_dist[d][xi][i] * p / sum_p;
          }
        }
        // while (bts2.get_next()) {
  ...
  void epoch_parallel_multideg(vector<double> &nt_vec, int d, int xi) {
    ...
    for (int i = 0; i < newsk[d][xi].size(); ++i) {
      ...
      BetaGenerator_highdeg bts1(newsk[d][xi][i], newsk_thres[d][xi][i], xi),
          bts2(newsk[d][xi][i], newsk_thres[d][xi][i], xi);
      int sum_iter = 0; // number of iteration
      int a_iter = 0;   // number of iteration
      int b_iter = 0;
      double sum_p = 0; // denominator
      vector<double> imm_p;
      vector<vector<double>> imm_now_results;
      ...
      if (sum_p == 0) {                   // for debug
        if (sum_iter > 0) {               // adjust the value
          ...
          std::cout << " n_layer2 " << temp_n_layer2;
          std::cout << " threshold_l1 " << temp_threshold_l1 << " * (" << xi
                    << " - 1)";
          temp_val -=
          ...
          if (temp_n_layer2 > 0) {
            ...
            std::cout << " threshold l2 " << temp_threshold_l2 << " -> "
                      << temp_val;
          }
        ...
        for (size_t j = 0; j < imm_p.size(); j++) {
          ...
          bts2.get_next();
          double p_check =
              get_p_from_beta_fcm(bts2, lambda, dist_old, n_old, xi, d);
          if (p_check != p) {
            std::cout << "P not equal? " << p << " " << p_check << std::endl;
          }
          b_iter++;
          for (size_t x = 0; x < imm_now_results[j].size(); x++) {
            nt_vec[imm_now_results[j][x]] += counter_dist[d][xi][i] * p / sum_p;
        ...
        // while (bts2.get_next()) {
        //   b_iter++;
        //   double p = get_p_from_beta_fcm(bts2, lambda, dist_old, n_old, xi,
        //   d); for (int j = 0; j < bts2.now_flow_num; ++j) {
        //     nt_vec[bts2.now_result[j]] += p / sum_p;
        //   }
        // }
        if (b_iter != a_iter) {
          std::cout << "What a != b " << a_iter << " " << b_iter << std::endl;
        }
      }
    }
    double accum = std::accumulate(nt_vec.begin(), nt_vec.end(), 0.0);
  ...
  void next_epoch() {
    ...
    for (int d = 0; d < DEPTH; ++d) {
      for (int xi = 1; xi <= num_thread[d]; ++xi) { // degree (xi)
        ...
        if (xi == 1) {
          thrd[iter_thread] =
              std::thread(&EM_FCM::epoch_parallel_singledeg, *this,
                          std::ref(nt_d_xi[d][xi]), d, xi);
        } else {
          thrd[iter_thread] =
              std::thread(&EM_FCM::epoch_parallel_multideg, *this,
                          std::ref(nt_d_xi[d][xi]), d, xi);
        }
        iter_thread += 1;
      }
    ...
    std::cout << "[EM_FCM - iter " << iter
              << "] Compute time : " << time.count() << std::endl;
