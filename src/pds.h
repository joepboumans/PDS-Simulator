...
class PDS {
public:
  std::unordered_map<FIVE_TUPLE, uint32_t, fiveTupleHash> true_data;
  string name = "NOT SET";
...
class PDS_FlowTuple {
public:
  std::unordered_map<FLOW_TUPLE, uint32_t, flowTupleHash> true_data;
  string name = "NOT SET";
  string trace_name = "NOT SET";
  uint32_t mem_sz;
  uint32_t rows;
  ...
  PDS_FlowTuple(string trace, uint32_t stage, uint32_t n) {
    this->trace_name = trace;
    this->n_stage = stage;
    this->n_struct = n;
  }
  PDS_FlowTuple(string trace, uint32_t stage, uint32_t n, uint32_t sz) {
    this->trace_name = trace;
    this->n_stage = stage;
    this->n_struct = n;
    this->mem_sz = sz;
  }
  ~PDS_FlowTuple() = default;
  virtual uint32_t insert(FLOW_TUPLE tuple) {
    std::cout << "Not implemented!" << std::endl;
    return -1;
  };
  virtual uint32_t remove(FLOW_TUPLE tuple) {
    std::cout << "Not implemented!" << std::endl;
    return -1;
  }
  virtual uint32_t lookup(FLOW_TUPLE tuple) {
    std::cout << "Not implemented!" << std::endl;
    return -1;
  }
  virtual void reset() {
